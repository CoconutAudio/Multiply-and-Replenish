...
{
    static Ort::Env environment { ORT_LOGGING_LEVEL_WARNING, "Multiply and Replenish" };
    return environment;
}
...
std::vector<std::string> OnnxSession::getOutputNames() const
{
    ...
    {
        Ort::AllocatorWithDefaultOptions allocator;
        const auto numOutputs = session->GetOutputCount();

        names.reserve (numOutputs);

        for (std::size_t outputIndex = 0; outputIndex < numOutputs; ++outputIndex)
            names.emplace_back (session->GetOutputNameAllocated (outputIndex, allocator).get());
    }
    catch (const Ort::Exception&)
    {
        names.clear();
                                          ...
                                          const std::vector<const char*>& outputNames) const
{
    ...
    try
    {
        ...
        for (const auto& input : inputs)
        {
            ...
            else if (input.boolData != nullptr)
            {
                inputValues.push_back (Ort::Value::CreateTensor<bool> (
                    memoryInfo,
                    const_cast<bool*> (input.boolData),
                    static_cast<std::size_t> (numElements),
                    input.shape.data(),
                    input.shape.size()));
            }
