                  ...
                  const SegmenterSettings& settings)
{
    ...
    note.semitones = weightedMedian (pitches, weights);
    note.originalSemitones = note.semitones;
                                     ...
                                     const SegmenterSettings& settings)
{
    ...
    for (std::size_t index = 0; index < firstFrames.size() && index < lastFrames.size(); ++index)
    {
        ...
        measureNote (note, melody, settings);

        note.sourceFirstFrame = note.firstFrame;
        note.sourceLastFrame = note.lastFrame;
                                ...
                                const SegmenterSettings& settings)
{
    ...
    for (auto index = 0; index < static_cast<int> (notes.size());)
    {
        ...
        const auto previousDistance = hasPrevious
            ? std::abs (notes[static_cast<std::size_t> (index - 1)].semitones - note.semitones)
        ...
        measureNote (neighbour, melody, settings);

        neighbour.sourceFirstFrame = neighbour.firstFrame;
        neighbour.sourceLastFrame = neighbour.lastFrame;
