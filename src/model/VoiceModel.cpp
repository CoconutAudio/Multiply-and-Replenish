                                              ...
                                              juce::String& error)
{
    ...
    RmvpeDetector::Configuration pitchConfiguration;
    pitchConfiguration.sampleRate = manifest.pitchSampleRate;
    pitchConfiguration.numPitchBins = manifest.numPitchBins;
    pitchConfiguration.centsOrigin = manifest.centsOrigin;
    pitchConfiguration.centsPerBin = manifest.centsPerBin;
    pitchConfiguration.centsReferenceHz = manifest.centsReferenceHz;
    pitchConfiguration.localAverageRadius = manifest.localAverageRadius;
    pitchConfiguration.salienceThreshold = manifest.salienceThreshold;
    pitchConfiguration.frameCountMultiple = manifest.frameCountMultiple;
    pitchConfiguration.minimumFrequencyHz = manifest.pitchMinimumHz;
    pitchConfiguration.maximumFrequencyHz = manifest.pitchMaximumHz;
    pitchConfiguration.mel = mel;
    pitchConfiguration.inputName = manifest.pitchEstimatorInput;
    pitchConfiguration.outputName = manifest.pitchEstimatorOutput;

    model->pitchDetector = std::make_unique<RmvpeDetector> (pitchConfiguration,
                                                            std::move (filterBankCopy),
                                                            model->pitchNetwork);
