                                                                 ...
                                                                 juce::String& error) const
{
    ...
    // Nothing is known about where the notes are to begin with; each pass reads the last one's answer.
    ...
    for (int step = 0; step < std::max (1, config.numRefinementSteps); ++step)
    {
        ...
        // std::vector<bool> is a bitfield, so the tensors are given plain arrays instead.
